...
def create_app(cfg: AppConfig, state: StateStore) -> FastAPI:
    ...
    def _verify_payload(idx: int):
        ...
        steps.append({"name": "当前配置", "ok": True, "details": f"base_url={used_base}\nproxy={used_proxy}"})
        # Step 1: 打开首页
        try:
            r = runner.http.get("/")
            step1_ok = (r.status_code == 200)
            steps.append({"name": "打开首页", "ok": step1_ok, "status": r.status_code, "details": f"GET / -> {r.status_code}"})
        except Exception as e:
            steps.append({"name": "打开首页", "ok": False, "details": str(e)})
            return acc, JSONResponse({"ok": False, "steps": steps})
        # Step 2: 登录/会话
        login_ok = False
        try:
            login_ok = runner.login()
            steps.append({"name": "登录/会话", "ok": login_ok, "details": "已登录" if login_ok else "未登录或失败"})
            storage.record_account_login(acc["id"], login_ok)
        except Exception as e:
            steps.append({"name": "登录/会话", "ok": False, "details": str(e)})
        # Step 3: 资料
        profile_ok = False
        profile_data = None
        if login_ok:
            try:
                ...
                steps.append({"name": "获取资料", "ok": ok2, "details": ("成功" if ok2 else data)})
                ...
                steps.append({"name": "获取资料", "ok": False, "details": str(e)})
